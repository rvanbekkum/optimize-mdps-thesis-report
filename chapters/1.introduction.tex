...
Although various algorithms for learning \acrshortpl{acr:mdp} from a dataset do already exist, the state-of-the-art lacks a method for setting the hyper-parameters of these algorithms so to best reflect the underlying system and maximize the performance in the execution of its tasks.
...
The optimization is performed by applying a technique known as \acrlong{acr:bo}, such that we define a probability distribution over functions to model the performance measure and iteratively sample parameter-settings towards a global maximizer of the performance.
