...
In recent years particularly \acrfullpl{acr:mdp} \cite{Boutilier1999} have become a significant popular formalism for modeling \acrshort{acr:dtp} problems. 
...
\begin{figure}[t]
	\centering
	\includegraphics[width=\textwidth]{high-level-planning-diagram}
...
Apart from this we explore techniques that takes advantage of techniques from the closely related field of \acrshort{acr:rl} to approach \acrshort{acr:sdm} problems where possible.
Also, the chapter reviews a technique that overcomes the in-feasibility of accurately defining transition probabilities by accounting for uncertainty in these probabilities.
