...
The reason the reward needs to be only obtainable once is to ensure that the expressions $V_\mathit{DTP}$ and $V_\mathit{SIM}$ are in the same range and can be therefore be used together.
The reward function $R_i$ for some task with goal state $i \in \mathcal{S}_G$ is then defined as follows:
\begin{equation}
R_i(s, a, s') = 
\begin{cases}
\hspace{4pt} \xi & \text{if } s' = i \land s \neq s'\\
\hspace{4pt} 0 & \text{otherwise}
\end{cases}
\end{equation}
...
\begin{equation}
\label{eq:vsim}
V_{\mathit{SIM}, (s_0, R)} = \gamma^{k} \cdot R[i]
\end{equation}
...
The objective function $f$ in our case is the $\texttt{LearningStep}:  \Theta \mapsto \mathbb{R}$ function which maps parameter-settings $\theta \in \Theta$ to real-valued performance yielded by \acrshortpl{acr:mdp}.
