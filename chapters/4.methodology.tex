...
\begin{algorithm}[t!]
	...
	\begin{algorithmic}[1]
		...
		\Ensure{\acrshort{acr:mdp} $\mathcal{M}$, Evidence set $\mathcal{D}$}%\acrshort{acr:mdp} $\mathcal{M} = (\mathcal{S}, s_0, A, \delta, R)$} \Comment{$s_0$ and $R$ are here irrelevant}
		...
		\Function{OptimizationStep}{$\theta, V_{\mathcal{M}}, \mathcal{D}, \mathit{gp}$}
			...
			\Let{$\theta$}{$\argmax_{x \in \mathcal{\Theta}} a(x \vert \mathcal{D}, \mathit{gp})$}	\Comment{Suggest a sample $\theta$ to evaluate next}
			\State\Return $\theta$
		\EndFunction
	\end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[t!]
	...
	\begin{algorithmic}[1]
		...
		\Statex
		\Function{Phase1}{}
		\State $\beta = 1.0$ \Comment{Compute $V_\mathcal{M}$ based on the value function, i.e., $V_\mathcal{M} = V_\mathit{DTP}$}
		...
		\State $\beta = 0.0$ \Comment{Compute $V_\mathcal{M}$ based on simulations, i.e., $V_\mathcal{M} = V_\mathit{SIM}$}
		...
		\Function{OptimizationStep2}{$\theta, V_\mathcal{M}, \alpha$}\Comment{The $\alpha$ parameter weighs the $\mathsf{EI}$ of the first phase.}
			...
			\State $\theta \gets \argmax_{x \in \Theta} \alpha\cdot\mathsf{EI}(x\vert\mathcal{D}_1, \mathit{gp}_1) + (1 - \alpha)\cdot\mathsf{EI}(x\vert\mathcal{D}_2, \mathit{gp}_2)$
		\EndFunction
	\end{algorithmic}
\end{algorithm}
\clearpage
...
The value of a learned model $\mathcal{M}$ is then only based on the value functions $V$ for a set of tasks $T_\mathcal{M}$ the system is expected to perform.
