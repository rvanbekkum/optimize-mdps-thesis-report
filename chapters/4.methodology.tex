...
We also use this application for evaluating our framework through the experiments that were carried out and are documented in \autoref{ch:experimental-results}.
...
As mentioned in \autoref{ch:introduction}, the problem this thesis is concerned with is the development of probabilistic models, which are used for obtaining plans for automated control of systems whose dynamics are stochastic.
As the development of such probabilistic models oft to be a difficult and time-costly process for a human designer, an appealing idea is to automate the model development process by applying learning algorithms on a dataset of execution traces of the system that is in need of automated control.
...
All by all, the problem we deal with is that of automating the task of learning a performance-maximizing \acrshort{acr:mdp} for a system that involves uncertainty that generalizes well over multiple tasks.
...
Although there are other potential applications as described in \autoref{ch:introduction}, this particular application can be used to illustrate the complete procedure of learning \acrshortpl{acr:mdp} from data and obtaining plans accordingly quite well.
...
One should note that a proper assessment of model value should be made based on the performance in simulations due to discrepancies that may exist between the model and real world.
In the following section we discuss how the proposed framework takes these aspects into account so that on convergence an \acrshort{acr:mdp} is obtained that reflects the real world and generalizes over multiple tasks quite well.
