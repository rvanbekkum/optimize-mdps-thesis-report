...
The objective is to use a dataset of execution traces of a system to attain an accurate system representation, which can be used to obtain plans for optimal automated control of an underlying system.
...
Finally, in \autoref{sec:cost-effective-optimization}, we define our extension of this base framework in the form of a \textit{multi-phase framework} consisting of three cost-incremental phases whose aim is to further improve on the effectiveness of the framework.
...
%TODO State clearly again our problem, what we are going to do and why we want to do it (but in little more detail where necessary than in the introduction).
%TODO Formalize the problem
%TODO Say something about related work moving to reinforcement learning -> issues with this (perhaps: RL takes long and involves 'dangers' as it is done in the real-world; BRL intractable)
As mentioned in \autoref{ch:introduction}, the problem this thesis is concerned with is the development of probabilistic models in the form of \acrshortpl{acr:mdp} which are used for obtaining plans for automated control of systems whose dynamics are stochastic.
As the development of such probabilistic models tend to be a difficult and time-costly process for a human designer, an appealing idea is to automate the model development process by applying learning algorithms on a dataset of execution traces of the system that is in need of automated control.
...
Although algorithms for learning \acrshortpl{acr:mdp} from a dataset do exist, the state-of-the-art lacks a method of setting the hyper-parameters $\theta$ of these algorithms so to retrieve a model that best reflects the underlying system.

Formalizing this problem statement gives us the following description:
...
All in all, the problem we deal with is that of automating the task of learning a performance-maximizing \acrshort{acr:mdp} for a system that involves uncertainty that generalizes well over multiple tasks.
This involves the need for a proper way of expressing the value of a learned \acrshort{acr:mdp} offering a fair comparison of different \acrshortpl{acr:mdp}.
Most importantly, a solution to this problem should take into account the costs of learning and evaluating these system models.
...
Accordingly, based on such a quantization into a state-space $\mathcal{S}$, one can learn a transition function $\delta: \mathcal{S} \times A \times \mathcal{S} \mapsto [0, 1]$ for an \acrshort{acr:mdp} applying the maximum-likelihood estimation as defined in \autoref{sec:learning-markov-models}.
...
Accordingly, an optimal policy $\pi^\ast$, defining the most appropriate action from any state, for this simple \acrshort{acr:mdp} is quite straightforward and is depicted in \autoref{fig:dummy-map-4}.
...
In the following section we discuss how the proposed framework takes these aspects into account so that on convergence an \acrshort{acr:mdp} is obtained that reflects the real world and generalizes over multiple tasks quite well. %TODO Update last sentence with new contents of chapter
...
All in all, the learning algorithm requires one to fix an action space $A$ and time step $t$ and gather a dataset of a sufficient number of observation-action pairs to describe the dynamics of the system under consideration.
...
\begin{algorithm}[t!]
	\caption{Model-Optimization Base Framework}
	\label{alg:base-framework}
	\begin{algorithmic}[1]
		\Require{Set of execution traces $E$, Parameter space $\Theta$, Set of tasks $T$, Action space $A$, Time-step $t$, Discount factor $\gamma \in [0, 1]$, Weight factor $\beta \in [0, 1]$, Acquisition function $u: \Theta \mapsto \mathbb{R}$}%Training set $\mathcal{O} = \{O_1, \ldots, O_k\}$, Clustering algorithm, hyper-parameter(s) $\theta$, actions $A$}
		...
		\State $\textit{gp} \gets GP(0, K)$ \Comment{Zero-mean \acrshort{acr:gp} with covariance $K$}
		...
		\State\Return 
		\Statex
		\Function{LearningStep}{$\theta$}
			\State $\mathcal{S} \gets \textproc{Quantization}(E, \theta)$
			\State $\delta \gets \textproc{MaxLikelihood}(E, A, \theta)$
			...
			\State $\mathit{steps} \gets \textproc{RunSim}(\mathcal{M}, \pi)$
			\State $V_\mathit{SIM} \gets V_\mathit{SIM} + \gamma \cdot R[s_g]$
			\EndFor
		...
		\Function{OptimizationStep}{$\theta, V_{\mathcal{M}}$}
			...
			\Let{$\theta$}{$\argmax_{x \in \mathcal{\Theta}} u(x \vert \mathcal{D})$}
			\State\Return $\theta$
		\EndFunction
	\end{algorithmic}
\end{algorithm}
...
In the remainder of this section we explain the working of this routine.
