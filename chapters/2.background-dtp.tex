...
Solving the set of linear equations in each iteration is a time-costly operation, especially for large state spaces, in comparison to the $O(\lvert\mathcal{S}\rvert^2\cdot\lvert A\rvert)$ time required for an iteration of \acrshort{acr:vi}.
The main advantage of \acrshort{acr:pi} can however be that for a finite space the algorithm converges in a finite number of steps when there is no more strict improvement, because only a finite number of stationary policies exist, while in \acrshort{acr:vi} the convergence depends on the threshold.
%In \acrshort{acr:vi} the convergence, however, depends on a threshold, which may not have been reached while an optimal policy was already found.
