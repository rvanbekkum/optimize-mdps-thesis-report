...
Putting these components together yields \autoref{def:markov-chain} below.

\begin{definition}
	\label{def:markov-chain}
	A discrete stationary first-order \textit{Markov Chain} is a 3-tuple $(\mathcal{S}, \mat{A}, \arr{\pi})$ where $\mathcal{S}$ is a finite set of states, $\mat{A}$ the transition (probability) matrix and $\arr{\pi}$ the initial state (probability) vector.
\end{definition}
...
Due to the close nature of \acrshortpl{acr:mdp} these approaches can be applied almost directly to learn transition probabilities of discrete-state \acrshort{acr:mdp}, although more data will be required due to the addition of actions.
...
\begin{figure}[t!]
	\centering
	\includegraphics[width=\textwidth]{mdp-planning-diagram-v3-cropped2}
...
\subsubsection{Model-Based Planning Techniques}	% Also referred to as Indirect approaches
...
These parts together form the \acrshort{acr:mdp} which is fed to a planning algorithm which is often referred to as \textit{\acrshort{acr:mdp} solver}.
From this solver, a policy $\pi$ is obtained, which is usually evaluated through simulations or automated model checking tools to check if the policy will satisfy the set of goals.%usually evaluated by applying it in one or more simulations, rather than using it in a real-world environment.
