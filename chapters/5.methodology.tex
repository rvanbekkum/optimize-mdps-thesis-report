...
\begin{figure}[t!]
\centering
\begin{tikzpicture}[->,>=stealth',auto,node distance=2.5cm,every label/.style=draw]
\tikzstyle{every state}=[fill=white,draw=black,text=black,scale=1]	% thick
\node[state,label={50:\small$+0$}] (s1) {};
\node[state,label={50:\small$+0$}] (s2) [below of=s1] {};
\node[state,label={50:\small$+0$}] (s3) [right of=s1] {};
