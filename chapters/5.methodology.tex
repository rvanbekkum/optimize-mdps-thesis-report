...
Then a possible expression of how well the agent executes its task is:
\begin{equation}
\label{eq:vdtp}
V_{\mathit{DTP},(s_0,R)} = V[s_0]
\end{equation}
where $s_0 \in \mathcal{S}$ is the initial state and $R$ the reward function of the \acrshort{acr:mdp}.
...
\begin{equation}
\label{eq:vcom}
\beta \cdot V_{\mathit{DTP}, (s_0, R)} + (1 - \beta) \cdot V_{\mathit{SIM}, (s_0, R)}
\end{equation}
where $\beta \in [0, 1]$ is a parameter which specifies the relative weight of $V_{\mathit{DTP}, t}$ against that of $V_{\mathit{SIM}, t}$ and where $s_0$ and $R$ are the initial state and reward function respectively.
...
Let us then define $R_G$ as a set of reward functions $R_i$ for each $i \in \mathcal{S}_G$ in which a one-time reward is received only in goal-state $i$.
