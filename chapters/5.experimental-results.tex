...
This gives a clearly shows that sufficient data is needed to avoid under-fitting and obtaining more stable results with discernible performance measures.
...
The experiment that employs the \acrshort{acr:mei} function, however, appears in-effective to find the same global maximum in multiple repetitions.
All compared, the \acrshort{acr:gp-ucb} appears most effective in sampling parameter-settings that result in \acrshortpl{acr:mdp} yielding high performance.
...
\afterpage{
	...
	\begin{figure}[t!]
		...
		\caption{Plots showing \acrshort{acr:gp} posterior after (left) and observed values in (right) 30 iterations of the base framework on the \texttt{uol\_bl} environment, $\beta = 0.0$, $k$-Means, $100\%$ of the dataset used, \acrshort{acr:mei} acquisition function used (Experiment 10).}
		\label{fig:exp10}
	\end{figure}
	% ID = 11
	\begin{figure}[t!]
